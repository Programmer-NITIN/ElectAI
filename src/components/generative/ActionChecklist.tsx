            ...
            <li key={item.id} className="px-4 py-3">
              ...
              >
                ...
                <div className="flex-1">
                  <span className={cn("text-sm font-medium", isChecked ? "text-gray-500 line-through" : "text-white")}>
                    {item.text}
                  </span>
                  {item.description && (
                ...
                {item.priority === "high" && (
                  <span className="text-xs px-2 py-0.5 bg-red-500/20 text-red-400 rounded-full">High</span>
                )}
                {item.url && (
                  <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300" aria-label={`Open ${item.text} link`} onClick={(e) => e.stopPropagation()}>
